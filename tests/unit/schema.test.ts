...
describe('content files', () => {
  ...
  it('has exactly seven project files', () => {
    expect(countMd('src/content/projects')).toHaveLength(7);
  });

  // The route is generated by stripping the numeric prefix from the filename, so a
  // case-study href that drifts from its file name becomes a dead link that nothing
  // else catches — renumbering the projects is exactly when that happens.
  it('points every case-study link at a slug its own file produces', () => {
    const dir = 'src/content/projects';
    for (const file of countMd(dir)) {
      const body = readFileSync(`${dir}/${file}`, 'utf8');
      const href = body.match(/href:\s*(\/work\/\S+)/)?.[1];
      if (!href) continue;
      expect(href).toBe(`/work/${file.replace(/^\d+-/, '').replace(/\.md$/, '')}`);
    }
  });
